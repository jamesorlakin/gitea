...
func CreateWebPushSubscription(userID int64, subscription *structs.WebPushSubscription) error {
	return createWebPushSubscription(x, userID, subscription)
}

func createWebPushSubscription(e Engine, userID int64, subscription *structs.WebPushSubscription) error {
...
func SendWebPushNotificationToUser(userID int64, payload *structs.WebPushPayload) error {
	...
	for _, userSubscription := range userSubscriptions {
		subscription := &structs.WebPushSubscription{
...
func SendWebPushNotification(subscription *structs.WebPushSubscription, payload *structs.WebPushPayload) (*http.Response, error) {
	...
	resp, err := webpush.SendNotification(pushPayload, webPushSubscription, &webpush.Options{
		VAPIDPublicKey:  setting.PushNotificationsPublicKey,
		VAPIDPrivateKey: setting.PushNotificationsPrivateKey,
		TTL:             30,
	})
	if err != nil {
		return resp, err
